...
public class cuttofu : MonoBehaviour
{
    ...
    void OnMouseDown() {
        ...
        if ((isOnBoardA()) && 
            ...
            Instantiate(platedTofuObj, getBowlCoords(), platedTofuObj.rotation);
            gameflow2.foodOnBoardA = false;
            Destroy(gameObject);
    ...
    bool isOnBoardA() {
        return transform.position == gameflow2.boardACoords + gameflow2.addCutTofuBoardCoords;
    }
    bool isOnBoardB() {
        return transform.position == gameflow2.boardBCoords + gameflow2.addCutTofuBoardCoords;;
    }
